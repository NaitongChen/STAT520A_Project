...
to_store = data.frame(matrix(NA, nrow = length(file_names), ncol = 6))
...
for (i in 1:(length(file_names)/2)) {
  ...
  m = dim(dat1)[2]
  for (j in 1:m) {
    to_store[2*i-1, 2*j - 1] = ess(dat1[,j]) # 1,3,5
    to_store[2*i, 2*j - 1] = ess(dat2[,j])
    to_store[2*i-1, 2*j] = mcse(dat1[,j])$se # 2,4,6
    to_store[2*i, 2*j] = mcse(dat2[,j])$se
  }
}